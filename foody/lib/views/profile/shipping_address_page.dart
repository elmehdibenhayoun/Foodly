...
import 'package:foody/models/address.dart';
...
class _ShippingAddressState extends State<ShippingAddress> {
  ...
  void _onSearchChanged(String searchQuery) async {
    if (searchQuery.isNotEmpty) {
      final url = Uri.parse(
          'https://maps.googleapis.com/api/place/autocomplete/json?input=$searchQuery&key=$googleApiKey');
  ...
  void _getPlaceDetails(String placeId) async {
    final url = Uri.parse(
        'https://maps.googleapis.com/api/place/details/json?place_id=$placeId&key=$googleApiKey');
    final response = await http.get(url);
    if (response.statusCode == 200) {
      ...
      for (var component in addressComponents) {
        if (component['type'].contains(postalCode)) {
              ...
              children: [
                GoogleMap(
                  ...
                  initialCameraPosition: CameraPosition(
                      target: _selectedPosition ??
                          const LatLng(33.566464, -7.6201082),
                      zoom: 50),
                  markers: _selectedPosition == null
                      ? {
                          Marker(
                            markerId: const MarkerId('Your Location'),
                            position: const LatLng(33.566464, -7.6201082),
                            draggable: true,
